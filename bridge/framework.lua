-- nblbnk_army - Bruecken-Adapter fuer ESX und QBCore
--
...
CreateThread(function()
  ...
  if not Army.Framework then
    print('^1[nblbnk_army]^7 Kein unterstuetztes Framework gefunden. ' ..
          'Erwartet wird es_extended oder qb-core. Die Ressource bleibt inaktiv.')
    return
  end

  if not acquireCoreObject() then
    print(('^1[nblbnk_army]^7 Framework %s erkannt, aber das geteilte Objekt ' ..
  ...
  Army.Ready = true
  print(('^2[nblbnk_army]^7 Framework erkannt: %s'):format(Army.Framework))
end)
...
if isServer then
  ...
  function Army.Notify(src, message, kind)
    TriggerClientEvent('nblbnk_army:notify', src, message, kind or 'inform')
  end
...
-- ===========================================================================
  ...
  RegisterNetEvent('nblbnk_army:notify', function(message, kind)
